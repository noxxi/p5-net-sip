...
=item generate_dtmf ( EVENT, DURATION, %ARGS )
...
return () if no new events where found or (EVENT,DURATION) if an event finished.

For performance reasons it is best to use rfc2833 if the peer supports it.

=back

=head1 TODO

actually implement extract_dtmf
