...
on the legs up. Thus it hash to be followed by a call to B<loop>.
...
with the newly created L<Net::SIP::Simple::Call> object as argument.

=item cb_final

Callback which will be called, after the call is established, e.g.
after receiving the ACK from the peer.
