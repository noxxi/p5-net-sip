...
If B<dst_addr> is not given needs to be computed from the URI.
Naturally this works only for requests.
See sub B<__resolve_uri> in the source code.

If the URI contains an IP address this will be used.

If B<domain2proxy> was given in the constructor it will be used
to lookup an outgoing proxy. 
If SIP domain for outgoing request is not in the mapping but
an upper domain is in the mapping then this entry will be used.

If no outgoing proxy could be found it will be looked up using 
a partial implementation of RFC3263, e.g. DNS SRV records will
be used (but not DNS NAPTR records).
