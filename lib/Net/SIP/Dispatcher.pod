...
Optional mapping between target SIP domain and leg to use, similar
to B<domain2proxy>. It is a reference to a hash where the key
is the domain and the value the L<Net::SIP::Leg> object.

=item leg2proxy

Optional mapping between legs and outgoing proxies. This is a 
reference to a list of \@pairs, where the first part of the
pair is the L<Net::SIP::Leg> object and the second part is the
C<< "ip:port" >> of the proxy.
...
=item resolve_uri ( URI, ADDR_REF, LEG_REF, CALLBACK )

Resolves URI to get the destination address and the outgoing leg.
ADDR_REF and LEG_REF are references which will get filled with
the computed values. 
...
If still no leg is known but B<domain2leg> it will try to get the leg
from this (but still no dst_addr). This option makes usually only
sense in connection with B<leg2proxy> and without B<domain2proxy>.

=item *

If no dst_addr is known but a leg (mostly when B<domain2leg>
is used) it will try to use B<leg2proxy> to find out the outgoing proxy
for the leg.
...
The outgoing leg will be computed based on B<dst_addr>.
