...
The method returns a \@list of new media in the format needed by
B<replace_media_listen> in L<Net::SIP::SDP>.
...
will cause all sessions for other transactions for the same call (identified
...
The method returns TRUE only if the activation succeeded.
It is usually called whenever a SDP body is given and on ACK requests.
...
It will return TRUE only if the session existed.
Stateless proxies should ignore the return code and forward the packet
nonetheless, because it's probably a retransmit.
...
This will return a list (not a reference) of C<< [fd,callback] >> pairs,
where fd is the file descriptor of the socket and callback is a callback
for B<invoke_callback> in L<Net::SIP::Util>.
